...
void Motion::setTimerCutoff(float time) {
  //sets the time to cut off motions

  timerCutoff = time;
}


void Motion::setUpdateFrequency(float dt) {

  this->dt = dt * 1000.0;
}


void Motion::cancelMotions() {

  runMotions = true;
}


void Motion::moveDist(float target, float maxSpeed, float timeout, float minSpeed) {
  //moves forward for target distance

  if (Timer::getTime() > timerCutoff && timerCutoff != -1) {
    timerCutoff = -1;
    return;
  }
  ...
  target += Units::tickToUnit((fl.position(vex::deg) + fr.position(vex::deg)) / 2.0, 2, foot);//Units::tickToUnit(parallel.position(vex::deg), 2, foot);

  do {

    uint64_t timestamp = vex::timer::systemHighResolution();

    if (runMotions) return;

    error = target - Units::tickToUnit((fl.position(vex::deg) + fr.position(vex::deg)) / 2.0, 2, foot);//Units::tickToUnit(parallel.position(vex::deg), 2, foot);
    
    float lateralOutput = Misc::clamp(lateral.tick(error), -fabs(maxSpeed) * 12000.0, fabs(maxSpeed) * 12000.0);
    if (fabs(lateralOutput) < minSpeed * 12000.0) lateralOutput = copysign(minSpeed * 12000.0, lateralOutput);
    ...
    timer += dt / 1000.0;
    wait((timestamp + dt + 1000.0 - vex::timer::systemHighResolution()) / 1000.0, vex::msec);
  }
  while (( Exit::error(error, 0.08)
        || Exit::velo(odometry.getSpeed(), 0.25))
...
void Motion::moveDist2(float target, float maxSpeed, float timeout) {
  ...
  float initialPos = Units::tickToUnit((fl.position(vex::deg) + fr.position(vex::deg)) / 2.0, 2, foot);
  ...
  do {
    ...
    error = target + initialPos - Units::tickToUnit((fl.position(vex::deg) + fr.position(vex::deg)) / 2.0, 2, foot);
    
    float lateralOutput = Misc::clamp(lateral.tick(std::copysign(999999, target)), -fabs(maxSpeed) * 12000.0, fabs(maxSpeed) * 12000.0);
...
void Motion::push(float speed, float timeout, float minVelo) {
  ...
  while ( (Exit::velo(odometry.getSpeed(), minVelo) && fabs(imu.roll(vex::deg)) < 8)
        && Exit::time(timer, timeout));
...
void Motion::push(float coord, bool isY, float speed, float timeout, float minVelo) {
  ...
  while (( Exit::velo(odometry.getSpeed(), minVelo)
        || start == cur)
        && Exit::time(timer, timeout));
...
void Motion::push2(float timeout, bool reverse, float minVelo) {
  //pushes until cannot move anymore
  ...
  do {
    ...
    float output = (reverse) ? -12000.0 : 12000.0;

    Misc::runDrivetrain(output, output, false);

    timer += 10;
    wait((timestamp + 11000.0 - vex::timer::systemHighResolution()) / 1000.0, vex::msec);
  }
  while (  Exit::velo(odometry.getSpeed(), minVelo)// && fabs(imu.roll(vex::deg)) < 10)
        && Exit::time(timer, timeout));
...
void Motion::push3(float coord, bool isY, float speed, float timeout, float minVelo) {
  //pushes until reaches coordinate and cannot move anymore
  ...
  do {
    ...
    Misc::runDrivetrain(12000.0 * speed, 12000.0 * speed, false);
...
void Motion::hangAlign(float x, float y, float speed) {
  //aligns the robot winch hang
  ...
  Pose end(x, y, -1);
  Pose pose(odometry.getPosX(), odometry.getPosY(), Misc::getHeading(true));
  ...
  do {
    ...
    pose.set(odometry.getPosX(), odometry.getPosY(), Misc::getHeading(true));

    float linearOutput = -12000.0 * speed;
    float headingOutput = Misc::clamp(turn.tick(pose.face(end, false)), -12000.0, 12000.0);

    float rescale = fabs(linearOutput) + fabs(headingOutput) - 12000.0;
    if (rescale > 0) linearOutput -= copysign(rescale, linearOutput);

    std::pair<float, float> outputs = Misc::reduceRatio(12000.0, linearOutput + headingOutput, linearOutput - headingOutput);
    Misc::runDrivetrain(outputs.first, outputs.second, false);

    timer += 10;
    wait((timestamp + 11000.0 - vex::timer::systemHighResolution()) / 1000.0, vex::msec);
  }
  while (timer < 1000 || Exit::velo(odometry.getSpeed(), 0.1));
...
void Motion::turnHeading(float target, int sides, float minSpeed, float maxSpeed, float timeout) {
  ...
  while (((minSpeed == 0 && Exit::velo(fabs(odometry.getAngularVelocity()), 3))
          || Exit::error(Misc::angleError(target, Misc::getHeading(false)), 5))
          && timer < timeout);
...
void Motion::turnPoint(float x, float y, int sides, float minSpeed, float maxSpeed, float timeout, bool reverse) {
  ...
  do {
    ...
    float heading = (reverse) ? Misc::getHeading(true) : Misc::getHeading(false);
    
    pose.set(odometry.getPosX(), odometry.getPosY(), heading);
...
void Motion::movePoint(float x, float y, float minSpeed, float maxSpeed, float timeout, bool settle, bool endIntake) {
  ...
  while (((minSpeed == 0 && Exit::velo(odometry.getSpeed(), 0.5))
        ...
        || !crossedOnce)
        && (!endIntake || Exit::intaked(intake1) || Exit::error(pose.distance(end), 0.5))
        && Exit::time(timer, timeout));
...
void Motion::movePose(float x, float y, float theta, float dLead, float gLead, float minSpeed, float maxSpeed, float timeout, float chasePower, bool settle, bool endIntake) {
  ...
  while (((minSpeed == 0 && Exit::velo(odometry.getSpeed(), 0.5))
        || !crossedOnce
        || Exit::error(pose.distance(end), 0.7))
        && (!endIntake || Exit::intaked(intake1) || Exit::error(pose.distance(end), 0.5))
        && Exit::time(timer, timeout));

  headingTarget = (maxSpeed < 0) ? theta - 180.0 : theta;

  //if (crossedOnce) printf("a\n");
  //if (!Exit::error(pose.distance(end), 0.7)) printf("f\n", pose.distance(end));
  //if (!Exit::time(timer, timeout)) printf("%d\n", timer);
...
void Motion::followPursuit(Path path, float timeout, float chasePower, bool reverse, bool endIntake) {
  ...
  while (((path.lastPoint().theta == 0 && Exit::velo(odometry.getSpeed(), 0.5)) 
        ...
        && (!endIntake || Exit::intaked(intake1))
        && Exit::time(timer, timeout));
