...
void TwoWheelInertialOdometry::setPose(float x, float y, float theta) {
...
Pose TwoWheelInertialOdometry::estimatePose(float dT) {
    //estimates the pose of the robot a certain amount of time ahead, circular arc approximation

    float w = Units::degToRad(getAngularVelocity());
    float v = getSpeed();
    float initialHeading = Units::degToRad(pose.theta);

    if (w == 0) return Pose(pose.x + v * sin(initialHeading) * dT, pose.y + v * cos(initialHeading) * dT, pose.theta);

    float r = v / w;

    float x = pose.x + r * cos(initialHeading) + r * cos(initialHeading + M_PI + w * dT);
    float y = pose.y - r * sin(initialHeading) + r * sin(initialHeading + w * dT);
    float theta = Units::radToDeg(initialHeading + w * dT);
